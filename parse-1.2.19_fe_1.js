 ...
 *
 * Updated May 2014: arne.sikstrom@frostyelk.se
 * Updated 1.2.19_fe_1.js: Removed all connection to FB.init(), the "status" option must be set by FB init beforehand.
 */
/* jshint -W069, -W097, -W117, -W041, -W030, -W058, -W054, -W033 */
(function(root) {
...
(function(root) {
  ...
  var provider = {
    authenticate: function(options) {
		...
		function storeOptions(options, authResponse) {
			if (options.success) {
				options.success(self, {
					id: authResponse.userID,
					access_token: authResponse.accessToken,
					expiration_date: new Date(authResponse.expiresIn * 1000 +
					  (new Date()).getTime()).toJSON()
				});
			} else {
				if (options.error) {
					options.error(self, authResponse);
				}
			}
		}
		...
		if (existingResponse) {
			//console.log("Already authenticated in Parse API: " + JSON.stringify(existingResponse));
            console.log("Already FB authenticated in Parse API");
			storeOptions(options, existingResponse);
		} else {
			//console.log("Doing FB.login in Parse API");
			FB.login(function(response) {
				if (response.authResponse) {
					console.log("Successful FB.login in Parse API");
					storeOptions(options, response.authResponse);
				} else {
					console.warn("Failed FB.login in Parse API");
					if (options.error) {
...
/* Async, callback is lost sometimes	  
		FB.getLoginStatus(function(response) {
			console.log("authenticate() FB.getLoginStatus() status: " + response.status);
			if (response.status === 'connected') {
				...
				storeOptions(options, response);
			} else if (response.status === 'not_authorized') {
				// the user is logged in to Facebook, 
				// but has not authenticated your app
				// 
				// TODO: Check if this is a valid state during authenticate
			} else {
...
*/	  
    ...
    restoreAuthentication: function(authData) {
		...
		// authData is fetched from Parse
		//console.log("Parse restoreAuthentication(). Parse stored authData: " + JSON.stringify(authData));
	  ...
	  if (authData) {
		...
		existingResponse = FB.getAuthResponse();
		if (existingResponse) {
			// User is logged in to FB
			if (existingResponse.accessToken == authData.access_token) {
				console.log("AccessToken FB = Parse.");
			} else {
				// Logged in but Parse has another token
				// TODO: How do we handle this?
				console.log("AccessToken FB != Parse.");

				//////////////////
				// Save the FB token to Parse?
				// Not so good if the token on Parse has a future expiration date
				var fixedDate = authData.expiration_date.substring(0,23);
				fixedDate = fixedDate + "Z";
				authData.expiration_date = fixedDate;
				var parseExpireTime = Parse._parseDate(authData.expiration_date).getTime();
				var fbExpireTime = existingResponse.expiresIn;
				if (parseExpireTime > fbExpireTime) {
					console.log("accessToken is newer on Parse.");
				} else {
					console.log("accessToken is newer in FB.");
				}
				
				//console.log("Parse.User: " + JSON.stringify(Parse.User.current()));
				if (Parse.User.current()) {
					var user = Parse.User.current();					
					var newAuthData = {
						facebook: {
							id: existingResponse.userID,
							access_token: existingResponse.accessToken,
							expiration_date: new Date(existingResponse.expiresIn * 1000 +
								(new Date()).getTime()).toJSON()
						}
					}

					//console.log("newAuthData: " + JSON.stringify(newAuthData));
					//console.log("Old Parse user AuthData: " + JSON.stringify(user.get("authData")));				
				}
				//////////////////
					
				
				/////////////////
				// Old style with FB.init
				// Not allowed in FB API April 2014
                // status options should be set correctly before
                /*
				var fixedDate = authData.expiration_date.substring(0,23);
				fixedDate = fixedDate + "Z"
				authData.expiration_date = fixedDate;
				...
				// console.log("New FB.init options: " + JSON.stringify(newOptions));
				if (initOptions.version != "v2.0") {
					//console.log("FB API != v2.0");
					//FB.init(newOptions); 
				} else {
					//console.log("FB API is probably v2.0");
				}			
                */
				//////////////////////
				
			}
			//console.log("FB authData exists: " + JSON.stringify(existingResponse));
		} else {
			//console.log("FB authData don't exist. Parse stored authData: " + JSON.stringify(authData));
		}

		return true; // All is well
      }
	  
      return true; // Return true if no authData on server
    },
